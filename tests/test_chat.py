...
async def test_handle_chat_message_creates_conversation():
    ...
    # db.execute sequence: Conv -> Fields -> Narrative -> History
    ...
    with patch("backend.chat._sync_search_qdrant", return_value=["Mock Chunk"]), \
        ...
        async def mock_run_in_executor(executor, func, *args):
            # If args are passed, it's the search function (program_id, query)
            if args:
                return ["Mock Chunk"]
            else:
                return "Mock Reply"
